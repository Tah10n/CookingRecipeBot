...
public class MessageHandler implements UpdateHandler {
    ...
    public MessageHandler(UserDAO userDAO, TelegramClient telegramClient, RecipeManager recipeManager, BotStateContextDAO botStateContextDAO, ActionFactory actionFactory, MessageTranslator messageTranslator) {
    ...
    private void handleDefaultState(Update update, String inputText) {
        ...
        List<Recipe> recipes = recipeManager.getRecipeDAO(user.getLanguage()).findRecipesByString(inputText);
    ...
    private SendMessage updateRecipe(Update update, SendMessage sendMessage, User user) {
        ...
        try {
            ...
            Recipe recipeToEdit = recipeManager.getRecipeDAO(user.getLanguage()).findRecipeById(recipeId);
            ...
            recipeManager.getRecipeDAO(user.getLanguage()).saveRecipe(recipeToEdit);
    ...
    private SendMessage addNewRecipe(Update update, SendMessage sendMessage, User user) {
        ...
        if (checkIsRecipeAlreadyExists(recipe, user.getLanguage())) {
            ...
            Recipe savedRecipe = recipeManager.getRecipeDAO(user.getLanguage()).saveRecipe(recipe);
    ...
    private boolean checkIsRecipeAlreadyExists(Recipe recipe, String language) {
        ...
        return recipeManager.getRecipeDAO(language).findRecipeByNameEqualsIgnoreCase(recipeName) != null;
    }
