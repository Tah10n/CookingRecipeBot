...
public class CallbackQueryHandler implements UpdateHandler {
    ...
    public CallbackQueryHandler(InlineKeyboardMaker inlineKeyboardMaker, TelegramClient telegramClient, RecipeManager recipeManager, BotStateContextDAO botStateContextDAO, UserDAO userDAO, ActionFactory actionFactory, MessageTranslator messageTranslator, ReplyKeyboardMaker replyKeyboardMaker) {
    ...
    private void yesForDeleteRecipe(CallbackQuery callbackQuery, long chatId, int messageId, User user) {
        ...
        recipeManager.getRecipeDAO(user.getLanguage()).deleteRecipe(recipeId);
    ...
    private void deleteRecipe(CallbackQuery callbackQuery, long chatId, User user) {
        String recipeId = callbackQuery.getData().substring(callbackQuery.getData().indexOf(":") + 1);
        String message = String.format(messageTranslator.getMessage(BotMessageEnum.DELETE_RECIPE_QUESTION_MESSAGE.name(), user.getLanguage()), recipeManager.getRecipeDAO(user.getLanguage()).findRecipeById(recipeId).getName());
    ...
    private void openAndCloseRecipe(CallbackQuery callbackQuery, long chatId, int messageId) {
        ...
        User user = userDAO.getUserById(userId);
        String recipeId = callbackQuery.getData().substring(callbackQuery.getData().lastIndexOf(":") + 1);
        Recipe recipe = recipeManager.getRecipeDAO(user.getLanguage()).findRecipeById(recipeId);
        if (recipe == null) {
    ...
    private void rateRecipe(CallbackQuery callbackQuery, long userId, long chatId, int messageId, int ratingFromUser) {
        ...
        Recipe recipe;
        recipe = recipeManager.getRecipeDAO(user.getLanguage()).findRecipeById(recipeId);
        ...
        recipeManager.getRecipeDAO(user.getLanguage()).saveRecipe(recipe);
