...
public class ActionFactory {
    ...
    public ActionFactory(BotStateContextDAO botStateContextDAO, TelegramClient telegramClient, UserDAO userDAO, RecipeManager recipeManager, ReplyKeyboardMaker replyKeyboardMaker, InlineKeyboardMaker inlineKeyboardMaker, MessageTranslator messageTranslator) {
    ...
    private @NotNull Runnable getFindRandomRecipeAction(User user, long chatId) {
        return () -> {
            ...
            botStateContextDAO.changeBotState(user.getId(), BotState.DEFAULT);
            Recipe randomRecipe = recipeManager.getRecipeDAO(user.getLanguage()).getRandomRecipe();
